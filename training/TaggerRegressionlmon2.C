...
void TaggerRegressionlmon2(
			      TString inDataNames   = "/scratch/EIC/Jarda/lowQ2_reco/trk-rev1b2.root",
			      TString outDataName   = "/scratch/EIC/Jarda/lowQ2_reco/trainedData.root",
			      ...
			      )
{
  ...
  dataloader->AddVariable( "intercept_pos_y[0]", "fit_position_y", "units", 'F' );
  dataloader->AddVariable( "intercept_pos_z[0]", "fit_position_z", "units", 'F' );
  dataloader->AddVariable( "intercept_vec_x[0]", "fit_vector_x",   "units", 'F' );
  dataloader->AddVariable( "intercept_vec_y[0]", "fit_vector_y",   "units", 'F' );
  ...
  dataloader->AddTarget( "true_el_momx/"+mcBeamEnergy );
  dataloader->AddTarget( "true_el_momy/"+mcBeamEnergy );
  dataloader->AddTarget( "true_el_momz/"+mcBeamEnergy );
 
  std::cout << "--- TMVARegression           : Using input files: " << inDataNames << std::endl;
 
  // Register the regression tree
 
  TChain* regChain = new TChain("event");
  ...
  TCut mycut = "@intercept_pos_y.size()==1&&(intercept_pos_y<1000||intercept_pos_y>-1000)"; // Make sure there's one reconstructed track in event
  ...
  TString layoutString("Layout=TANH|64,TANH|32,LINEAR");
  
  TString trainingStrategyString("TrainingStrategy=");
  trainingStrategyString +="LearningRate=1e-4,Momentum=0,MaxEpochs=10000,ConvergenceSteps=2000,BatchSize=64,TestRepetitions=1,Regularization=None,Optimizer=ADAM";   
